...
#
# ARMADILHA DO SCRIPT QUE SE AUTO-MODIFICA (ja aconteceu duas vezes): o `git pull`
# do passo 1 pode substituir ESTE arquivo enquanto ele esta rodando. O bash le o
# script em pedacos, conforme executa, entao trocar o arquivo no meio faz a leitura
# continuar num deslocamento errado — na pratica, uma versao ANTIGA do deploy
# continuou rodando e os passos novos (backup, check) simplesmente nao aconteceram,
# em silencio.
#
# A protecao e a funcao `main` no fim: o bash precisa parsear a funcao INTEIRA antes
# de comecar a executa-la, entao o corpo do deploy fica todo em memoria e a troca do
# arquivo no disco nao afeta esta execucao. Ao mexer aqui, manter tudo dentro de
# `main()` e a chamada `main "$@"` como ULTIMA linha do arquivo.
set -e
